...
class StoreConsole extends StatefulWidget {
  StoreConsole({
    Key key,
  }) : super(key: key);

  @override
  _StoreConsoleState createState() => _StoreConsoleState();
}

class _StoreConsoleState extends State<StoreConsole> with AfterLayoutMixin {
  Store _store;
  bool _notFound;
  ...
  void afterFirstLayout(BuildContext context) {
    ...
    if (!_auth.checkRole('store_manager')) {
      Get.offAll(Home(
        autoRedirect: true,
      ));
    }
  ...
  void getStore() async {
    ...
    Firestore.instance
        .collection('stores')
        .where('managerId', isEqualTo: auth.fUser.uid)
        .snapshots()
        .listen((doc) {
      if (doc.documents.length > 0) {
        setState(() {
          _store = Store.fromSnapShot(doc.documents[0]);
        });
        Firestore.instance
            .collection('orders')
            .where('storeId', isEqualTo: _store.id)
          ...
          if (docs.documents.length > 0) {
            ...
            docs.documents.forEach((doc) {
              if (doc['statusMessage'] == 'Pending Confirmation') {
                _pendingOrders++;
              }
            });
            setState(() {});
