...
class StoreOrderDetails extends StatefulWidget {
  ...
  StoreOrderDetails({Key key, this.order, this.orderId}) : super(key: key);
...
class _StoreOrderDetailsState extends State<StoreOrderDetails> {
              ...
              ? Column(
                  children: <Widget>[
                    ...
                    Card(
                      child: ListTile(
                        title: Text('Status'),
                        subtitle: Text(_order?.statusMessage),
                      ),
                    ),
                    ...
                    _order?.statusMessage != 'Pending Confirmation'
                        ? Card(
                            child: ListTile(
                              title: Text(_order.picker == null
                                  ? 'Assign Picker Manually'
                                  : 'Change Picker'),
                              subtitle: Text(_order.picker == null
                                  ? 'Select Picker'
                                  : '${_order.picker.name} (${_order.picker.mobile.toString().substring(3)})'),
                              onTap: () async {
                                User picker = await Get.to(
                                  AdminStorePickers(
                                ...
                                if (picker != null) {
                                  ...
                                  Firestore.instance
                                    ...
                                    'pickerIds':
                                        FieldValue.arrayUnion([picker.uid]),
                                  }).then((value) async {
                                    print('Picker Assigned');
                                    setState(() {
                                      _order.picker = picker;
                                    });
                                    final HttpsCallable callable =
                                        CloudFunctions.instance
                                            .getHttpsCallable(
                                      functionName: 'notificationByUser',
                                    );
                                    ...
                                    try {
                                      var res =
                                        ...
                                        'title': "New Order",
                                        'body':
                                            "Accept The Order #${_order.orderNumber}",
                                        'landing_page': "picker_orders",
                                        'image': "",
                                        'id': _order.id
                                      });
                    ...
                    _order?.statusMessage != 'Pending Confirmation'
                        ? Card(
                            child: ListTile(
                              title: Text(_order.driver == null
                                  ? 'Assign Driver'
                                  : 'Change Driver'),
                              subtitle: Text(_order.driver == null
                                  ? 'Select Driver'
                                  : '${_order.driver.name} (${_order.driver.mobile.toString().substring(3)})'),
                              onTap: () async {
                                User driver = await Get.to(
                                  AdminStoreDrivers(
                                    storeId: _order.storeId,
                                    isSelectDriver: true,
                                  ),
  ...
  Widget _bottomNavigation() {
    ...
    return _order?.statusMessage != 'Pending Confirmation'
        ? SizedBox()
        : Container(
                        ...
                        onPressed: _isBusy
                            ...
                            : () async {
                                ...
                                Firestore.instance
                                  ...
                                  setState(() {
                                    _isBusy = false;
                                    _order.statusMessage = 'Order Confirmed';
                                  });
                                  final HttpsCallable callable =
                                      CloudFunctions.instance.getHttpsCallable(
                                    functionName: 'notificationToPickers',
                                  );
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      image: new DecorationImage(
                        fit: BoxFit.fill,
                        image: CachedNetworkImageProvider(e.item.image != null
                            ? e.item.image.thumb
                            : Utils.getImageLinkBySku(e.item.sku)),
                      ),
                    ),
